...
**Обязательные права доступа**: `crm`, `disk`, `im`, `imbot`, `user`, `user_brief`, `pull`, `placement`, `userfieldconfig`
...
- **Разрешения (скоупы)**: `crm`, `disk`, `im`, `imbot`, `user`, `user_brief`, `pull`, `placement`, `userfieldconfig`
...
BITRIX_REST_ENDPOINT - REST endpoint портала для Node backend (`https://portal.bitrix24.ru/rest`).
