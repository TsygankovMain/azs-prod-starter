...
const onClick = async () => {
  ...
  try {
    ...
    if (result.ok && result.code) {
      ...
      // лёг в очередь ретрая (сеть/5xx), и flushPending() дожмёт её сама,
      // как только в следующий раз появится токен (fix round, BLOCKING 3).
      toast.info('Диагностика сохранена на устройстве и уйдёт, когда появится связь.')
    } else if (result.rejected) {
      // Re-review fix: 4xx — сервер отклонил бандл окончательно (битая
      // версия, неизвестный триггер, превышен потолок). Повтор здесь не
      // поможет — ни этот бандл, ни следующий такой же сервер не примет.
      toast.info('Диагностику не удалось принять. Опишите проблему в чате поддержки.')
    } else {
      // Троттлинг — недавно уже отправляли. Здесь повтор действительно
      // поможет, в отличие от случая выше.
