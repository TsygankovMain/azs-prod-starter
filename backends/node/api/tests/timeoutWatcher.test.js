...
test('timeoutWatcher: expired без причины + BITRIX_APP_CODE → кнопка «Указать причину» с deep-link', async () => {
  const doborNotifyCalls = [];
  ...
  try {
    ...
    assert.match(btn.LINK, /reason/, 'ссылка должна содержать /reason/');
    assert.match(btn.LINK, /30/, 'ссылка должна содержать id отчёта (30)');
  } finally {
