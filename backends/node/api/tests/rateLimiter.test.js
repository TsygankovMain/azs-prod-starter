...
// Раунд правок 2 (ревью нашло Important): цикл в takeOne() требует tokens>=1,
// чтобы дойти до break, а refill() держит tokens не выше burst (Math.min).
// При burst<1 (например 0.5) это условие недостижимо математически ни за
// какое время — acquire() зависнет навсегда, и, так как вызовы сериализованы
// через общую queue на весь процесс, застрянут вообще все последующие
// вызовы от всех трёх воркеров. Старый guard (`burst > 0`) пропускал 0.5 как
// валидное значение; новый (`burst >= 1`) обязан его отклонять на входе.
test('createRateLimiter отклоняет burst меньше 1 — иначе acquire() зависнет навсегда', () => {
  assert.throws(
    () => createRateLimiter({ ratePerSec: 2, burst: 0.5 }),
    /burst must be at least 1/,
    'burst<1 делает tokens>=1 недостижимым в цикле takeOne() — это должно быть отклонено на входе, а не приводить к вечному зависанию acquire()'
  );
});
