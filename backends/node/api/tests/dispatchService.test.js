...
test('pre-computed scheduledAt is used verbatim, jitter NOT re-applied', async () => {
  ...
  const service = createDispatchService({
    ...
    nowFn: () => new Date('2026-05-03T00:00:00.000Z'),
...
test('dispatchCandidate: клавиатура содержит COMMAND-кнопку причины при наличии BITRIX_APP_CODE', async () => {
  ...
  try {
    ...
    // BUG-019 v2: reason button is ACTION:SEND, not LINK/COMMAND
    ...
    assert.equal(reasonButton.ACTION, 'SEND', 'кнопка причины должна быть ACTION=SEND (BUG-019 v2)');
    // REASON-BTN-TEXT: ACTION_VALUE теперь человеческая фраза, без «/reason N»
    assert.ok(String(reasonButton.ACTION_VALUE).toLowerCase().includes('причин'), 'ACTION_VALUE — человеческий текст про причину');
    assert.ok(!String(reasonButton.ACTION_VALUE).includes('/reason'), 'в кнопке нет технического /reason');
    assert.equal(reasonButton.LINK, undefined, 'кнопка причины не должна иметь LINK');

    // NOTIF-1: при фоллбэке на notify кнопка теряется — текстовый путь /reason
    // должен сохраниться через fallbackSuffix.
    assert.ok(notifyCalls[0].fallbackSuffix, 'notifyDispatch должен получить fallbackSuffix');
    assert.match(notifyCalls[0].fallbackSuffix, /\/reason 42/, 'fallbackSuffix содержит /reason <reserve.id>');
  } finally {
