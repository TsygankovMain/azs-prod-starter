...
test('generateDailyPlan: 2 global candidates, valid workWindow → 1 in-window row each (window wins over dispatchTimes)', async () => {
  const store = makeFakePlanStore();
  const summary = await generateDailyPlan({
    planDate: '2026-06-03',
    candidates: candidates2,
    settings: makeSettings(),   // dispatchTimes + valid workWindow 07:00–14:00
    planStore: store
  });
  // Новое правило: для глобальных АЗС рабочее окно — приоритетный источник,
  // dispatchTimes игнорируется → ровно 1 точка на АЗС внутри окна.
  ...
  for (const call of store.calls) {
    const m = Number(call.baseTime.slice(0, 2)) * 60 + Number(call.baseTime.slice(2));
    assert.ok(m >= 7 * 60 && m <= 14 * 60, `baseTime ${call.baseTime} в окне 07:00–14:00`);
  }
});

test('generateDailyPlan (legacy fallback): 2 global candidates, NO workWindow → 2 fixed-times rows each → 4 upserts', async () => {
  const store = makeFakePlanStore();
  const summary = await generateDailyPlan({
    planDate: '2026-06-03',
    candidates: candidates2,
    settings: makeSettings({ report: { dispatchTimes: ['09:00', '12:00'], dispatchJitterMinutes: 15, workWindow: undefined } }),
    planStore: store
  });
...
test('generateDailyPlan: each upsert has correct planDate, azsId, baseTime', async () => {
  const store = makeFakePlanStore();
  // Проверяем форму upsert-записей на legacy fixed-times пути (без окна),
  // где baseTime берётся ровно из dispatchTimes и применяется jitter.
...
test('S8-A2: АЗС без профиля (null) + валидное workWindow → 1 точка в окне (окно приоритетнее dispatchTimes)', async () => {
  const store = makeFakePlanStore();
  // settings с профилем, но АЗС НЕ в нём; глобальное workWindow 07:00–14:00 валидно
  await generateDailyPlan({
    planDate: '2026-06-20',
  ...
  // Новое правило: для глобальной АЗС рабочее окно — приоритетный источник,
  // устаревший dispatchTimes игнорируется → ровно 1 точка внутри окна.
  assert.equal(store.calls.length, 1, 'ровно 1 точка в рабочем окне');
  const m = Number(store.calls[0].baseTime.slice(0, 2)) * 60 + Number(store.calls[0].baseTime.slice(2));
  assert.ok(m >= 7 * 60 && m <= 14 * 60, `baseTime ${store.calls[0].baseTime} в окне 07:00–14:00`);
});

test('S8-A2: АЗС без профиля (null), БЕЗ workWindow → глобальные dispatchTimes (legacy fallback регресс-гард)', async () => {
  const store = makeFakePlanStore();
  // нет валидного окна → старое поведение: точки из глобального dispatchTimes
  const settings = makeSettingsWithProfileA();
  settings.report.workWindow = undefined;
  await generateDailyPlan({
    planDate: '2026-06-20',
    candidates: [{ azsId: 'azs-global', adminUserId: 202 }],
    settings,
...
test('S8-A2→A3: АЗС в профиле режима B → генерирует 1 primary точку в окне[0], не падает', async () => {
  // Placeholder-тест A2 «пропускается TODO A3» заменён реальной проверкой после реализации A3.
  const store = makeFakePlanStore();
  const settingsWithB = {
    timezone: 'Europe/Moscow',
    report: {
    ...
    dispatchProfiles: [{
      ...
      config: {
        ...
        escalateUntilDone: false   // только 1 окно, без эскалации
      }
    }]
  };
  ...
  // A3 реализован: 1 primary точка в единственном окне
  assert.equal(store.calls.length, 1, 'режим B генерирует 1 primary точку');
  assert.equal(store.calls[0].entryType, 'primary', 'entryType = primary');
  assert.equal(store.calls[0].windowIndex, 0, 'windowIndex = 0');
  // executeAt в окне 06:00–10:00 MSK (UTC+3 → 03:00–07:00 UTC)
  const executeUTCHour = store.calls[0].executeAt.getUTCHours();
  assert.ok(executeUTCHour >= 3 && executeUTCHour <= 7,
    `executeAt UTC часы=${executeUTCHour} должны быть в [3, 7] (06:00–10:00 MSK)`);
});
...
test('S8-A2: несколько АЗС — часть в профиле A, часть без — каждая по своему источнику', async () => {
  ...
  // Всего 3 записи: 2 от профильной (slots A, без изменений) + 1 от глобальной (рабочее окно приоритетнее)
  assert.equal(store.calls.length, 3, 'профиль A: 2 слота + глобальная: 1 точка в окне = 3 точки');

  const profileCalls = store.calls.filter((c) => c.azsId === 'azs-profile');
  const globalCalls = store.calls.filter((c) => c.azsId === 'azs-global');

  // Профиль A не затронут изменением — по-прежнему 2 точки из slots
  ...
  // Глобальная АЗС: рабочее окно приоритетнее → 1 точка внутри окна 07:00–14:00
  assert.equal(globalCalls.length, 1, 'глобальная АЗС — 1 точка в рабочем окне');
  const gm = Number(globalCalls[0].baseTime.slice(0, 2)) * 60 + Number(globalCalls[0].baseTime.slice(2));
  assert.ok(gm >= 7 * 60 && gm <= 14 * 60, `глобальная baseTime ${globalCalls[0].baseTime} в окне 07:00–14:00`);
});
