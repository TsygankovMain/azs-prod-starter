...
## Sprint 8 «Расписание + Бренды» — 2026-06-15

### ВЫКАЧЕНО В ПРОД (master 3b77df2)

**Бренды (внешний доступ к фото):**
- Таблицы `brand` + `brand_azs` для связи АЗС с брендом (1:1)
- API CRUD: `/api/brands` (admin-only)
- Интеграция с Disk: `disk.folder.getexternallink` для папки бренда
- Роутинг новых фото в папку бренда; пароль вручную в Диске (Вариант A)
- UI: `brands.client.vue`, компонент `AzsMultiSelect`, карточка на главной

### ГОТОВО К ДЕПЛОЮ (branch feature/sprints-stability-ux, не выкачено)

**Профили рассылки (dispatchProfiles[]):**
- Режим A: слоты + джиттер (текущее поведение)
- Режим B: окна + эскалация (повтор в следующем окне при просрочке, дедлайн=конец окна)
- Миграция dispatch_plan → dispatchProfiles в settings.json
- UI редактирования профилей в настройках
- 791 тест зелёных; интеграционно проверены
