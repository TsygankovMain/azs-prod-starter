...
**Отложенная проверка слота — обязанность именно этого воркера.** Task 5 принимает фото со `slot_verified = FALSE`, когда список требуемых фото неизвестен и Битрикс недоступен, и откладывает проверку «до публикации». Воркер — единственное место, где эта публикация происходит, и до находки ревью Task 6 эта обязанность не была назначена никому: колонка заводилась, проставлялась, и на этом всё. Обещание без исполнителя — ровно та тихая потеря, против которой весь план.

Перед публикацией задачи со `slot_verified = FALSE` воркер обязан дорезолвить список требуемых фото (`readRequiredPhotos`, кэш из Task 1 к этому моменту почти всегда прогрет) и:

- слот в списке есть → публиковать как обычно, проставить `slot_verified = TRUE`;
- слота в списке нет → `markFailed` с внятной причиной. Это не потеря: байты остаются, сторож сообщит человеку, и решение примет он, а не мы молча;
- список по-прежнему не резолвится → **не менять ничего**, оставить задачу на следующий заход. Отказ по неизвестности — это ровно то превращение видимого отказа в невидимую потерю, которое запрещено.
...
> **ЭТА ЗАДАЧА ЧИНИТ CRITICAL, ВНЕСЁННЫЙ TASK 5. Идёт сразу за ней, до воркеров.**
>
> Task 5 перевёл приём фото на `photoQueueStore.accept()`, который **не заполняет `disk_folder_id`** — эту колонку заполняет только `markPublished()`, то есть факт публикации в Битриксе. А `POST /:id/submit` до сих пор требует хотя бы один `diskFolderId > 0`, иначе бросает `ReportSyncError('report_folder_missing', 502)` — причём **до** `setReportStatus({status:'done'})`, единственного места в кодовой базе, ставящего этот статус.
>
> Следствие, воспроизведённое скриптом: оператор грузит фото → 200, тут же жмёт «сдать» → **502, отчёт навсегда остаётся `in_progress`**. Во время простоя портала это не разрешится никогда: заполнить `disk_folder_id` значит сходить в Битрикс, а он и есть то, что лежит.
>
> Это ровно тот вред, который вся затея устраняет, только на шаг позже: не «фото не принято», а «смена не закрывается».
>
> **Почему не поймали тесты:** успешный путь `/:id/submit` не покрыт **ни одним** тестом из 1146. Единственный тест, который его трогает, проверяет ветку 409 при нехватке фото. Написать этот тест — часть задачи, а не факультатив.
>
> **Правка:** `/:id/submit` обязан считать комплект по **локальному** признаку — все обязательные слоты приняты (`publish_state IN ('accepted','published')`), — а не по наличию `disk_folder_id`. Факт реальной публикации сверяют воркер и сторож, и он **не должен блокировать закрытие смены**: дедлайн считается по `operator_completed_at`, времени оператора, а не по времени, когда Битрикс переварил файл.
